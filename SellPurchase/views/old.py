...
def farmer_login(request):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            error = "yes"
    return render(request, 'farmer_login.html', locals())
...
def regfarmers(request):
    if not request.user.is_authenticated:
        return redirect('admin_login')
    farmer = Farmer.objects.all()
    return render(request, 'admin/registerfarmer.html', locals())


def deletefarmer(request,pid):
    if not request.user.is_authenticated:
        return redirect('admin_login')
    farmer = Farmer.objects.get(id=pid)
    farmer.delete()
    return redirect('regfarmers')
